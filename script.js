...
const optionClickCounts = {};
let labelCheckResults = { a: false, b: false, c: false, d: false };

additionalQuizData.forEach(question => {
    const selectedOptionIndex = userAnswers[question.id - 1];
    if (selectedOptionIndex !== undefined) {
        const selectedOption = question.options[selectedOptionIndex];
        const label = selectedOption.label;

        // Increment the count for each label
        optionClickCounts[label] = (optionClickCounts[label] || 0) + 1;

        // Check if the count for any label reaches 6 or more
        if (optionClickCounts[label] >= 6) {
            labelCheckResults[label] = true;
        }
    }
});
    ...
    if (totalScore >= 230) {
        ...
        if (optionLessThan6) {
            ...
            /**advice (${totalScore})  (${optionClickCounts})**/
...
function downloadPDF() {
    ...
    // Main styling to match the attached design
    elementCopy.style.position = 'relative';
    elementCopy.style.padding = '20px';
    elementCopy.style.border = '2px solid #000';
    elementCopy.style.backgroundColor = '#ffffff';
    elementCopy.style.textAlign = 'justify'; // Justify the text
    elementCopy.style.fontFamily = "'Noto Serif Bengali', serif"; // For Bengali text if available

    // Header section similar to image design
    const headerDiv = document.createElement('div');
    headerDiv.style.textAlign = 'center';
    headerDiv.style.fontSize = '18px';
    headerDiv.style.fontWeight = 'bold';
    headerDiv.style.marginBottom = '10px';
    headerDiv.textContent = 'CONGRATULATIONS FOR SUCCESSFUL COMPLETION OF YOUR SELF-ASSESSMENT';
    elementCopy.insertBefore(headerDiv, elementCopy.firstChild);

    // Bengali title text
    const bengaliTitleDiv = document.createElement('div');
    bengaliTitleDiv.style.textAlign = 'center';
    bengaliTitleDiv.style.fontSize = '16px';
    bengaliTitleDiv.style.fontWeight = 'bold';
    bengaliTitleDiv.style.marginBottom = '20px';
    bengaliTitleDiv.textContent = 'পেশাগত লক্ষ্যের সংক্ষিপ্ত রূপরেখা';
    elementCopy.insertBefore(bengaliTitleDiv, elementCopy.firstChild);
    ...
    patientInfoDiv.style.fontSize = '14px';
    /*patientInfoDiv.innerHTML = `<strong>Name:</strong> ${userName} <br> <strong>Phone:</strong> ${userPhone} <br> <strong>ID:</strong> ${uniqueNumber}`;*/
    patientInfoDiv.innerHTML = `<strong>ID:</strong> ${uniqueNumber}`;
    elementCopy.insertBefore(patientInfoDiv, elementCopy.firstChild);
    ...
    // Adding a line separator above the footer section
    const lineSeparator = document.createElement('hr');
    lineSeparator.style.marginTop = '20px';
    lineSeparator.style.border = '1px solid #000';
    elementCopy.appendChild(lineSeparator);

    // Set options for generating the PDF
    const options = {
        margin: [10, 10, 10, 10],
        filename: fileName,
